...
if ! $curl_cmd --data "$(jq --null-input --arg username "${UNIFI_USERNAME}" --arg password "${UNIFI_PASSWORD}" '{"username": $username, "password": $password}')" "${UNIFI_URL}/api/login" > /dev/null; then
  echo Login failed
  exit 1
fi

# ask controller to do a backup, response contains the path to the backup file 
path=$($curl_cmd --data 'json={"cmd":"backup","days":"-1"}' "${UNIFI_URL}/api/s/${site_id}/cmd/system" | sed -n 's/.*\(\/dl.*unf\).*/\1/p')

# download the backup to the destinated output file 
$curl_cmd "${UNIFI_URL}$path" -o "$output/$filename"

# logout 
$curl_cmd "${UNIFI_URL}/logout"
